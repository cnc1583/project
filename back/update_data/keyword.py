...
# 프롬프트 설정
refined_system_prompt ="""Extract the most important keywords from the given news article titles, listing them in descending order of importance. The output must be in Korean and strictly formatted as "keyword1, keyword2, ...". Do not output anything other than what I instructed."""
...
batch_job = client.batches.create(
    ...
    config=types.CreateBatchJobConfig(
        display_name="keywording_labeling"
    )
)
...
# 7. 결과 파싱 및 DB 업데이트
# ==========================================
print("6. DB 업데이트 시작 (clusters 테이블 keywords 컬럼)...")
        ...
        for line in f:
            try:
                ...
                if 'response' in res and 'candidates' in res['response']:
                    generated_keywords = res['response']['candidates'][0]['content']['parts'][0]['text'].strip()
                    
                    cursor_cluster.execute(
                        "UPDATE clusters SET keywords = ? WHERE id = ?", 
                        (generated_keywords, c_id)
                    )
                    update_count += 1
                else:
                    # 에러 응답 등은 패스
